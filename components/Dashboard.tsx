...
export default function Dashboard({ mes, ano }: DashboardProps) {
    const { data: resumoPessoas, isLoading: loadingPessoas } = useQuery<ResumoPessoaDTO[]>({
        queryKey: ['resumoPessoas', mes, ano],
        queryFn: async () => (await axios.get(`http://localhost:8080/compras/resumo?mes=${mes}&ano=${ano}`)).data
    })

    const { data: resumoCartoes, isLoading: loadingCartoes } = useQuery<ResumoCartaoDTO[]>({
        queryKey: ['resumoCartoes', mes, ano],
        queryFn: async () => (await axios.get(`http://localhost:8080/cartoes/resumo?mes=${mes}&ano=${ano}`)).data
    })

    if (loadingPessoas || loadingCartoes) return <div className="h-48 animate-pulse bg-gray-200 rounded-xl mb-6"></div>

    const totalGeral = resumoPessoas?.reduce((acc, item) => acc + item.total, 0) || 0

    return (
        <div className="w-full max-w-2xl mb-8 grid gap-8">
             
             {/* Bloco 1: Totais */}
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-blue-600 p-6 rounded-2xl text-white shadow-lg shadow-blue-200 flex flex-col justify-between h-32 relative overflow-hidden group">
                     {/* ...Conteúdo do Total Geral... */}
                     <div className="text-3xl font-bold mt-2 text-white">
                        {totalGeral.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
                    </div>
                     <div className="text-white text-xs mt-auto opacity-90 font-medium">Total acumulado</div>
                </div>
                <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex flex-col justify-center h-32">
             ...
             <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                    {resumoCartoes?.map(c => {
                         ...
                         return (
                            <div key={c.apelido} className="p-4 rounded-xl border border-gray-100 bg-gray-50/50 hover:bg-white hover:shadow-md transition-all duration-300 group">
                                {/* ... Conteúdo do Card do Cartão ... */}
