...
export default function CadastrosPage() {
    ...
    // Pessoas - CORRIGIDO: Agora usa crases
    const { data: pessoas } = useQuery<Pessoa[]>({ 
        queryKey: ['pessoas', user?.id], 
        queryFn: async () => { 
            if(!user?.id) return []; 
            return (await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/pessoas`, config)).data 
        }, 
        enabled: !!user 
    })
    ...
    const salvarPessoa = useMutation({ 
        mutationFn: async () => { 
            if (editandoPessoaId) {
                return axios.put(`${process.env.NEXT_PUBLIC_API_URL}/pessoas/${editandoPessoaId}`, { nome: nomePessoa }, config);
            } else {
                // CORRIGIDO: Agora usa crases
                return axios.post(`${process.env.NEXT_PUBLIC_API_URL}/pessoas`, { nome: nomePessoa }, config);
            }
        }, 
        onSuccess: () => { 
    ...
    const deletarPessoa = useMutation({ 
        mutationFn: async (id: number) => axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/pessoas/${id}`, config), 
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ['pessoas'] }), 
        onError: () => alert("Erro ao apagar.") 
    })

    // Cartões - CORRIGIDO: Agora usa crases
    const { data: cartoes } = useQuery<Cartao[]>({ 
        queryKey: ['cartoes', user?.id], 
        queryFn: async () => { 
            if(!user?.id) return []; 
            return (await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/cartoes`, config)).data 
        }, 
        enabled: !!user 
    })
    ...
    const salvarCartaoMutation = useMutation({ 
        mutationFn: async () => { 
            ...
            if (editandoCartaoId) {
                return axios.put(`${process.env.NEXT_PUBLIC_API_URL}/cartoes/${editandoCartaoId}`, payload, config);
            } else {
                // CORRIGIDO: Agora usa crases
                return axios.post(`${process.env.NEXT_PUBLIC_API_URL}/cartoes`, payload, config);
            }
        }, 
        onSuccess: () => { 
            queryClient.invalidateQueries({ queryKey: ['cartoes'] }); 
            setEditandoCartaoId(null); 
            setFormCartao({ apelido: '', limite: '', dia: '', donoId: '' }) 
        } 
    })

    const deletarCartao = useMutation({ 
        mutationFn: async (id: number) => axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/cartoes/${id}`, config), 
